...
router.get('/', async (req, res, next) => {
  try {
    const where = req.user.role === 'WORKER'
      ? { createdById: req.user.userId, deletedAt: null }
      : { deletedAt: null }
...
router.get('/trash', requireRole('ADMIN', 'MANAGER'), async (req, res, next) => {
  try {
    const calculations = await prisma.calculation.findMany({
...
router.post('/:id/restore', requireRole('ADMIN', 'MANAGER'), async (req, res, next) => {
  try {
    ...
    if (!calculation.deletedAt) throw new ApiError(400, 'Расчёт не в корзине')
    const restored = await prisma.calculation.update({
      where: { id: req.params.id },
      data: { deletedAt: null },
      include: includeAll,
    })
    await logAudit(req.user.userId, req.user.fullName || 'Администратор', 'CALCULATION_RESTORED', restored.id, restored.title, null)
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    if (!calculation || calculation.deletedAt) throw new ApiError(404, 'Not found')
  ...
  try {
    const calculation = await prisma.$transaction(async (tx) => {
      ...
      return tx.calculation.update({
        where: { id: created.id },
        data: { title: `Заказ №${String(created.orderNumber).padStart(3, '0')}` },
        include: includeAll,
      })
    })
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    if (calculation.createdById !== req.user.userId) {
      await notify(calculation.createdById, 'Расчёт удалён', `Администратор переместил ваш расчёт «${calculation.title}» в корзину`)
    }

    await logAudit(req.user.userId, req.user.fullName || 'Администратор', 'CALCULATION_DELETED', calculation.id, calculation.title, null)
    res.json({ success: true })
  } catch (err) { next(err) }
})
