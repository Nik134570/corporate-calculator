...
class _AdminScreenState extends State<AdminScreen>
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (_tabController.index == 0) _loadPendingCount();
    });
  }

  Future<void> _loadPendingCount() async {
    try {
      final reviews = await getIt<CalculatorRepository>().getPendingReviews();
      if (mounted) setState(() => _pendingCount = reviews.length);
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            const Tab(icon: Icon(Icons.calculate_outlined), text: 'Расчёты'),
            const Tab(icon: Icon(Icons.people_outlined), text: 'Пользователи'),
            const Tab(icon: Icon(Icons.category_outlined), text: 'Справочники'),
          ],
        ),
        actions: [
          IconButton(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          _ReviewsTab(onReviewHandled: _loadPendingCount),
...
class _ReviewsTab extends StatefulWidget {
  final VoidCallback? onReviewHandled;
  const _ReviewsTab({this.onReviewHandled});
  ...
  void _confirmDelete(BuildContext context, CalculationModel calc) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('«${calc.title}» будет перемещён в корзину. Восстановить его можно будет в разделе «Корзина».'),
        actions: [
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        _CatalogSection(title: 'Корзина', icon: Icons.delete_outline_rounded,
