  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading
          ...
          : _materials.isEmpty
              ? Center(child: Text('Нет материалов', style: AppStyles.subtitleStyle))
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        title: Text(m['name'] ?? '', style: AppStyles.titleMedium),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            IconButton(
                              icon: Icon(Icons.edit_outlined, color: AppStyles.primary, size: 20),
                              onPressed: () => _showDialog(context, m),
                            ),
                            IconButton(
                              icon: Icon(Icons.delete_outline, color: AppStyles.danger, size: 20),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading
          ? const Center(child: CircularProgressIndicator())
          : _templates.isEmpty
              ? Center(child: Text('Нет наименований', style: AppStyles.subtitleStyle))
              : ListView.builder(
                  padding: AppStyles.listPadding,
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      shape: AppStyles.cardShape(10),
                      child: ListTile(
                        title: Text('$matName ${t['thickness']}', style: AppStyles.titleMedium),
                        subtitle: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('${t['unitPrice']} ₽/м²', style: AppStyles.subtitleStyle),
                            const SizedBox(height: 2),
                            Wrap(
                              spacing: 4,
                              children: [
                                if (tempered) _Chip('Закалка', Colors.blueGrey),
                                if (procCount > 0) _Chip('Обработок: $procCount', AppStyles.primary),
                                if (pwCount > 0) _Chip('Штучных: $pwCount', AppStyles.success),
                              ],
                            ),
                          ],
                        ),
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('${item[priceKey]} $priceSuffix', style: AppStyles.subtitleStyle),
                            if (tmplNames.isNotEmpty) ...[
                              ...
                              Text('Нет привязанных наименований',
                                  style: AppStyles.captionStyle.copyWith(color: AppStyles.warning)),
                            ],
                          ],
                        ),
                        isThreeLine: true,
...
class _DiscountsAdminScreenState extends State<DiscountsAdminScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppStyles.background,
      appBar: AppBar(title: const Text('Скидки'), backgroundColor: AppStyles.appBarBg, foregroundColor: AppStyles.appBarFg, elevation: 0),
      floatingActionButton: FloatingActionButton(onPressed: () => _showDialog(context, null), child: const Icon(Icons.add)),
      body: _loading
          ? const Center(child: CircularProgressIndicator())
          : _items.isEmpty
              ? Center(child: Text('Нет скидок', style: AppStyles.subtitleStyle))
              : ListView.builder(
                  padding: AppStyles.listPadding,
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      shape: AppStyles.cardShape(10),
                      child: ListTile(
                        title: Text(_discountLabel(item), style: AppStyles.titleMedium),
                        subtitle: tmplNames.isNotEmpty
                            ? Text('Изделия: ${tmplNames.join(', ')}', style: AppStyles.captionStyle, maxLines: 2, overflow: TextOverflow.ellipsis)
                            : Text('Нет привязанных наименований', style: AppStyles.captionStyle.copyWith(color: AppStyles.warning)),
                        isThreeLine: tmplNames.isNotEmpty,
                        trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                          IconButton(icon: Icon(Icons.edit_outlined, color: AppStyles.primary, size: 20), onPressed: () => _showDialog(context, item)),
                          IconButton(icon: Icon(Icons.delete_outline, color: AppStyles.danger, size: 20), onPressed: () => _delete(context, item['id'])),
                        ]),
                      ),
                    );
                  },
